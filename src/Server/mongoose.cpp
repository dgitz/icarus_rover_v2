...
#define MG_TCP_RECV_BUFFER_SIZE (256*1024)
...
char expiresDate[64];
  ...
  if ((pd->file.fp = fopen(path, "rb")) == NULL) {
    ...
    time(&curTime);
    futureTime= curTime + (7*24*60*60);
    setTime= gmtime(&futureTime);
    sprintf(expiresDate, "%s, %d %s %d %d:%d:%d GMT", day[setTime->tm_wday], setTime->tm_mday, month[setTime->tm_mon], setTime->tm_year+1900, setTime->tm_hour, setTime->tm_min, setTime->tm_sec);
    mg_printf(nc,
              "Date: %s\r\n"
              "Last-Modified: %s\r\n"
              "Accept-Ranges: bytes\r\n"
              ...
              "Cache-Control: max_age=31536000, public\r\n\r\n",
              current_time, last_modified, (int) mime_type.len, mime_type.p,
              (size_t) cl, etag, expiresDate);
