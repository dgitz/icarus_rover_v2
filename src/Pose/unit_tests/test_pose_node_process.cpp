...
TEST(Template, Process_MeasureExecutionRate)
{
	PoseNodeProcess *process = initializeprocess(2);
	process = readyprocess(process);
	double time_to_run = 200.0;
	double dt = 0.001;
	...
	uint64_t counter = 0;
	printf("Running Pose Model Timing Tests.  Please wait to finish...\n");
	...
	printf("t:%4.2f %ld %4.2f %4.2f (Hz)\n",current_time,counter,etime,(double)counter/etime);
}
