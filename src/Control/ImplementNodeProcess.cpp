...
eros::diagnostic ImplementNodeProcess::set_config_filepaths(std::string t_filepath)
{
...
eros::diagnostic ImplementNodeProcess::load_configfile(std::string path)
{
	eros::diagnostic diag = root_diagnostic;
...
eros::diagnostic  ImplementNodeProcess::finish_initialization()
{
    eros::diagnostic diag = root_diagnostic;
	diag = load_configfile(config_filepath);
	diag = update_diagnostic(diag);
	//Initialize Cylinder Controllers
	{
...
eros::diagnostic ImplementNodeProcess::new_devicemsg(const eros::device::ConstPtr& device)
{
	eros::diagnostic diag = update_diagnostic(SOFTWARE,INFO,NOERROR,"Updated Device");
	return diag;
}
std::vector<eros::diagnostic> ImplementNodeProcess::new_commandmsg(const eros::command::ConstPtr& t_msg)
{
...
std::vector<eros::diagnostic> ImplementNodeProcess::check_programvariables()
{
	std::vector<eros::diagnostic> diaglist;
	eros::diagnostic diag = root_diagnostic;
	bool status = true;
