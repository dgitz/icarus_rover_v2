...
/*! \brief Constructor
 */
HatControllerNodeProcess::HatControllerNodeProcess(std::string _base_node_name,
...
icarus_rover_v2::diagnostic HatControllerNodeProcess::update(double dt)
{
	...
	{
		status = false;
		if(time_sincelast_pps > 20.0)
		{
			diag.Level = ERROR;
		}
		else if(time_sincelast_pps > 5.0)
		{
			diag.Level = WARN;
		}
...
/*! \brief Run Unit Test
 */
std::vector<icarus_rover_v2::diagnostic> HatControllerNodeProcess::run_unittest() {
	std::vector<icarus_rover_v2::diagnostic> diaglist;
