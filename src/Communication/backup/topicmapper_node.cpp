...
{
	logger->log_debug("Got joy!");
}
void Dumb_Callback(const std_msgs::Bool::ConstPtr& msg)
{
	logger->log_debug("dumb.");
}
...
bool initialize(ros::NodeHandle nh)
{
    ...
    //ros::Subscriber joy_sub = nh.subscribe<sensor_msgs::Joy>("/Diagnostics_GUI/joystick",1000,Joystick_Callback);
    ros::Subscriber dumb_sub = nh.subscribe<std_msgs::Bool>("/bool",1000,Dumb_Callback);
	...
	if(topicmapfile_loaded == true)
	{
		...
		for(int i = 0; i < TopicMaps.size();i++)
		{
			if(TopicMaps.at(i).input_topic_type == "sensor_msgs/Joy")
			{
				...
				//joy_sub = nh.subscribe<sensor_msgs::Joy>(TopicMaps.at(i).input_topic_name,1000,Joystick_Callback);
				/*joystick_subs = new ros::Subscriber[resource_topics.size()];
