...
bool publish_transforms()
{
	...
	joint_state.name[0] ="baserotate";
	joint_state.position[0] = 0.0;
	joint_state.name[1] ="armrotate";
	joint_state.position[1] = 0.0;
	joint_state.name[2] ="forearmrotate";
	joint_state.position[2] = 0.0;
	joint_pub.publish(joint_state);
	joint_state.name[3] ="gripperrotate";
	joint_state.position[3] = 0.0;
	joint_pub.publish(joint_state);
	joint_state.name[4] ="gripperpivot";
...
bool initialize(ros::NodeHandle nh)
{
    ...
    for (ros::master::V_TopicInfo::iterator it = master_topics.begin() ; it != master_topics.end(); it++)
    {
    	...
    	std::size_t found = info.name.find("leftarm_joint");
