...
void SerialIO::ResetConnection()
{
	...
	serial_port = ResetSerialPort();

	if((serial_port > 0) and (stream_request_in_progress == false))
	{
		reset_in_progress = false;
		stream_request_in_progress = true;
		printf("Serial port reopened, requesting stream: %d\n",update_type);
			...
			if (bytes_read > 0) {
				...
				while (i < bytes_read)
				{
					...
					if (packet_length > 0) {
						packets_received++;
						update_count++;
						if(update_count > 255) { update_count = 0; }
