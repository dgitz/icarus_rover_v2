...
SerialPort *SerialIO::ResetSerialPort()
{
    if (debug_level >= 1)
    {
        printf("[%4.2f] Resetting Serial Port.\n",run_time);
    }
    if (serial_port != 0)
    {
        try
...
void SerialIO::ResetDevice()
{
    if (debug_level >= 1)
    {
        printf("[%4.2f] ResetDevice.\n", run_time);
    }
    ResetSerialPort();
}
...
int SerialIO::DecodePacketHandler(char *received_data, int bytes_remaining)
{
    ...
    if(debug_level >= 3)
    {
        printf("[%4.2f] Packet: %s\n",run_time,received_data);
    }
    if ((packet_length = IMUProtocol::decodeYPRUpdate(received_data, bytes_remaining, ypr_update_data)) > 0)
    {
        ...
        if (debug_level >= 2)
        {
            printf("[%4.2f] Updating YawPitchRoll.\n",run_time);
        }
    ...
    else if ((packet_length = AHRSProtocol::decodeAHRSPosTSUpdate(received_data, bytes_remaining, ahrspos_ts_update_data)) > 0)
    {
        ...
        if (debug_level >= 2)
        {
            printf("[%4.2f] Updating TS AHRSPosData.\n", run_time);
        }
    ...
    else if ((packet_length = AHRSProtocol::decodeAHRSPosUpdate(received_data, bytes_remaining, ahrspos_update_data)) > 0)
    {
        ...
        if (debug_level >= 2)
        {
            printf("[%4.2f] Updating AHRSPosData.\n", run_time);
        }
    ...
    {
        notify_sink->SetAHRSData(ahrs_update_data, run_time);
        if (debug_level >= 2)
        {
            printf("[%4.2f] Updating AHRSData.\n", run_time);
        }
    ...
    else
    {
        if (debug_level >= 1)
        {
            printf("[%4.2f] Unknown Packet.\n", run_time);
        }
        packet_length = 0;
    }
    //printf("packet: %d %s\n",packet_length,received_data);
    return packet_length;
}
double SerialIO::measure_time_diff(struct timeval a,struct timeval b)
{
	double a_sec = (double)a.tv_sec + (double)(a.tv_usec)/1000000.0;
	double b_sec = (double)b.tv_sec + (double)(b.tv_usec)/1000000.0;
	return a_sec-b_sec;
}
double SerialIO::convert_timestamp(struct timeval t)
{
...
void SerialIO::Run()
{
    ...
    while (!stop)
    {
        ...
        gettimeofday(&last_time,NULL);
        run_time+= last_timedelta;
        if (debug_level >= 3)
        {
            printf("[%4.2f] Loop: Run.\n", run_time);
        }
        flush_timer += last_timedelta;
        if (flush_timer > 10.0)
        {
            if (debug_level >= 1)
            {
        ...
        try
        {
            ...
            /*if (updater == 100)
            {
                ...
                printf("Writing: %s %d\n",stream_command,cmd_packet_length);
                serial_port->Write(stream_command, cmd_packet_length);
                if (debug_level >= 4)
                {
                    printf("[%4.2f] a2.\n", run_time);
...
uint8_t SerialIO::GetUpdateCount()
{
    return update_count;
}
